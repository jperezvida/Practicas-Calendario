...
const DayPanel: React.FC<Props> = ({ date, entries, currentUser, onClose, onEntryChange }) => {
  ...
  // Aceptamos 'vacaciones' en el estado
  const [type, setType] = useState<'diario' | 'plan' | 'vacaciones'>('diario');
  ...
  const handleEdit = (entry: Entry) => {
    ...
    // CORREGIDO: Ahora permite cargar 'vacaciones' sin resetearse a 'diario'
    if (entry.type === 'plan' || entry.type === 'diario' || entry.type === 'vacaciones') {
        setType(entry.type);
    } else {
        setType('diario');
    }
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      ...
      await dbService.addEntry({
        ...
        type,
        completed: false
      } as any); // Cast as any por si falta algún campo opcional legacy
    }
    ...
    <div className="absolute top-0 right-0 w-full md:w-96 h-full bg-white shadow-2xl border-l border-gray-200 z-40 flex flex-col animate-slide-in">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50/30">
        {entries.length === 0 ? (
          ...
          entries.map(entry => {
            ...
            const isVacation = entry.type === 'vacaciones'; // Detectamos vacación
            const people = entry.participants && entry.participants.length > 0 
              ? entry.participants 
              : [entry.person];
            ...
            return (
              <div key={entry.id} className={`group relative bg-white rounded-2xl p-4 border border-gray-200 shadow-sm transition-all ${entry.completed ? 'opacity-50' : ''}`}>
                  ...
                  <div className="flex items-center -space-x-2 overflow-hidden py-1">
                    {people.map((pName, idx) => {
                      ...
                      return (
                        <div 
                          key={idx} 
                          className="relative z-10 w-6 h-6 rounded-full border-2 border-white shadow-sm flex items-center justify-center text-[9px] text-white font-bold" 
                    ...
                    <div className="ml-3">
                      {/* ETIQUETA TIPO */}
                      {isVacation ? (
                         <span className="text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-widest border bg-purple-50 text-purple-600 border-purple-100">
                           ✈️ LIBRE
                         </span>
                      ) : (
                ...
                <p className={`text-sm text-gray-700 leading-relaxed font-medium ${entry.completed ? 'line-through' : ''}`}>
                  {entry.text}
                </p>
                ...
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition bg-white/90 backdrop-blur-sm rounded-lg p-1 shadow-sm border border-gray-100">
                  <button onClick={() => addToGoogleCalendar(entry)} className="p-1.5 text-gray-400 hover:text-blue-600" title="Añadir a Google Calendar">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"/></svg>
                  </button>
                  <button onClick={() => handleEdit(entry)} className="p-1.5 text-gray-400 hover:text-pink-600">
      ...
      <div className="p-6 border-t bg-white">
        <div className="mb-4 space-y-3">
          ...
          {/* Selector Tipo CON BOTÓN DE VACACIONES */}
          <div className="flex bg-gray-100 p-1 rounded-xl gap-1">
            ...
            <button 
              onClick={() => {
                setType('vacaciones');
                if(!text) setText('Vacaciones'); // Autorellenar si está vacío
              }} 
              className={`flex-1 py-2 text-[10px] font-bold rounded-lg transition-all ${type === 'vacaciones' ? 'bg-white shadow text-purple-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
              🏖️ LIBRE
            </button>
          </div>
          ...
          {/* Input Texto */}
          <textarea 
            className="w-full p-4 rounded-xl border border-gray-200 text-sm font-medium bg-white focus:ring-2 focus:ring-pink-500 outline-none resize-none transition h-24"
            placeholder={type === 'diario' ? "¿Qué habéis hecho hoy?" : type === 'plan' ? "¿Qué plan tenéis?" : "Detalles de las vacaciones..."}
            value={text}
        ...
        <button 
          onClick={handleSave}
          disabled={!text.trim()}
          className="w-full py-3 bg-pink-600 text-white rounded-xl font-bold text-sm shadow-lg hover:bg-pink-700 transition transform active:scale-95 disabled:opacity-50"
        >
